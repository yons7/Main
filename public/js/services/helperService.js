myApp.factory('HelperService', function ($http, $locale, Ressources) {
    ...
    _helperService.padLeft = function (n, c, len) {
        var x = ('' + n).length;
    ...
    _helperService.calculateEndBalance = function (bankStatement) {
        if (bankStatement && bankStatement.transactions && bankStatement.transactions.length > 0) {
    ...
    _helperService.displayDashboardRecette_Depense = function (listRecipe, listSpending) { 
        var dashboard = [];
        var Lastligne = { 'month' : 'Total', 'recipe' : 0 , 'spending' : 0 , 'diff' :  0};

        for (i = 0; i < 12; i++) {            
            var ligne = { 'month' : '', 'recipe' : 0 , 'spending' : 0 };
            ligne.month = $locale.DATETIME_FORMATS.MONTH[i];
            for (var obj in listRecipe) {
                if (listRecipe[obj].date.month === i + 1) {
                    ligne.recipe = Math.round((ligne.recipe + listRecipe[obj].amount) * 100) / 100;
                }
            }
            Lastligne.recipe = Math.round((Lastligne.recipe + ligne.recipe) * 100) / 100;
            for (var obj in listSpending) {
                if (listSpending[obj].date.month === i + 1) {
                    ligne.spending = Math.round((ligne.spending + listSpending[obj].amount) * 100) / 100;
                }
            }           
            Lastligne.spending = Math.round((Lastligne.spending + ligne.spending) * 100) / 100;

            ligne.diff = ligne.recipe - ligne.spending;
            dashboard.push(ligne);
        }
        Lastligne.diff = Lastligne.recipe - Lastligne.spending;
        dashboard.push(Lastligne);
        return dashboard;
    }
    
    _helperService.displayDashboardSpending = function (listSpending, listNatureSpend) { 
        var dashboard = [];
        var Lastligne = { 'title': 'Total du mois', 'month' : [], 'totalNature' : 0 };
        for (var i in listNatureSpend) {
            var ligne = { 'title': '', 'month' : [], 'totalNature' : 0 };
            ligne.title = listNatureSpend[i].name;
            for (j = 0; j < 12; j++) {
                var sousTotalMois = 0;
                for (var obj in listSpending) {
    ...
    _helperService.displayDashboardkm = function (listkm, listScale) {
        var dashboard = [];
        //calculer les taux
        for (var km in listkm) {
            for (var scale in listScale) {
                if (listkm[km].vehicle.type === listScale[scale].type && listkm[km].vehicle.cv === listScale[scale].cv) {
                    if (listkm[km].km < listScale[scale].interval.t2.from) {
                        listkm[km].amount = Math.round(((Math.round((listScale[scale].interval.t1.rate * listkm[km].km) * 100) / 100) + listScale[scale].interval.t1.constant) * 100) / 100;
                        listkm[km].taux = '(' + listScale[scale].interval.t1.rate + ' * Kms) + ' + listScale[scale].interval.t1.constant;
                    }
                    if (listkm[km].km >= listScale[scale].interval.t2.from && listkm[km].km <= listScale[scale].interval.t2.to) {
                        listkm[km].amount = Math.round(((Math.round((listScale[scale].interval.t2.rate * listkm[km].km) * 100) / 100) + listScale[scale].interval.t2.constant) * 100) / 100;
                        listkm[km].taux = '(' + listScale[scale].interval.t2.rate + ' * Kms) + ' + listScale[scale].interval.t2.constant;
                    }
                    if (listkm[km].km > listScale[scale].interval.t2.to) {
                        listkm[km].amount = Math.round(((Math.round((listScale[scale].interval.t3.rate * listkm[km].km) * 100) / 100) + listScale[scale].interval.t3.constant) * 100) / 100;
                        listkm[km].taux = '(' + listScale[scale].interval.t3.rate + ' * Kms) + ' + listScale[scale].interval.t3.constant;
                    }
                }
            }
        }
        ...
        for (i = 0; i < 12; i++) {
            ...
            dashboard.push({ 'date': $locale.DATETIME_FORMATS.MONTH[i], 'libelle' : 'Total mois', 'kms': totalKms, 'taux': '', 'montant': totalMontant });
        }
    ...
    _helperService.getLastPiece = function (list) {
        var compteurPiece = 0;
        for (var i = 0; i < list.length ; i++) {
            var num = list[i].num_justification.substring(7, 10);
            if (parseInt(num) && compteurPiece < num) {
                compteurPiece = num;
            }
        }
        compteurPiece++;
    ...
    _helperService.getCurrentState = function (list) {
        if (list.length > 0)
            var currentBankStatement = list[0];
        else
            var currentBankStatement = { 'status' : -1 }
        
        switch (currentBankStatement.status) {
            case 0:
                currentBankStatement.etat = 'En cours';
                currentBankStatement.color = 'warning';
                currentBankStatement.text = 'Merci de valider les justifiactions des recettes/dépenses pour le mois en cours.';
                break;
            case 1:
                currentBankStatement.etat = 'Recettes terminé';
                currentBankStatement.color = 'info';
                currentBankStatement.text = 'Les recettes sont justifiées pour le mois en cours, merci de terminer la jusitifiaction des dépenses.';
                break;
            case 2:
                currentBankStatement.etat = 'Recettes/Dépenses terminé';
                currentBankStatement.color = 'info';
                currentBankStatement.text = 'Les recettes et les dépenses sont justifiées pour le mois en cours.';
                break;
            case 3:
                currentBankStatement.etat = 'Exercice cloturé';
                currentBankStatement.color = 'success';
                currentBankStatement.text = 'Merci de contactez votre administrateur pour modifier l\'exercice cloturé.';
                break;
            default:
                currentBankStatement.etat = 'A faire';
                currentBankStatement.color = 'danger';
                currentBankStatement.text = 'Merci d\'importer le relevé bancaire du mois en cours.';
                break;
        }
        return currentBankStatement;
    };
